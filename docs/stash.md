...
3. Call `stashService.giveOrStash(player, items)` and chain logging as needed.
...
plugin.stashService().ifPresent(stash -> {
    stash.giveOrStash(player, List.of(rewardItem))
        .whenComplete((result, throwable) -> {
            ...
            if (result.stashedItems() > 0) {
                player.sendMessage(Component.text("Your stash caught the overflow.", NamedTextColor.GOLD));
            }
        });
});
```
